...
import { Menu, X, Search, History, Shield, User as UserIcon, LogOut, Home } from 'lucide-react';
import { useToast } from './components/ToastContext';

type View = 'home' | 'main' | 'history' | 'admin' | 'account';
...
const App: React.FC = () => {
  ...
  if (!user) {
    if (showAuth) {
      return <AuthView onLogin={handleLogin} onBack={() => setShowAuth(false)} initialIsLogin={authMode === 'login'} />;
    }
    return <LandingPage onLaunch={() => { setAuthMode('login'); setShowAuth(true); }} onRegister={() => { setAuthMode('register'); setShowAuth(true); }} />;
  }
  ...
  return (
    <div className="min-h-screen bg-obsidian text-silver flex font-sans relative overflow-hidden">
      ...
      <main className="flex-grow md:ml-20 lg:ml-64 transition-all duration-300 relative z-10">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-64 bg-obsidian border-r border-glass p-6 flex flex-col animate-slide-in-left">
              ...
              <nav className="space-y-2 flex-1">
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${view === 'home' ? 'bg-cobalt/10 text-cobalt border border-cobalt/30' : 'text-muted hover:text-white hover:bg-white/5'}`}
                >
                  <Home size={20} />
        ...
        {view === 'home' ? (
          <LandingPage onLaunch={() => setView('main')} isLoggedIn={true} hideNavbar={true} onRegister={() => {}} />
